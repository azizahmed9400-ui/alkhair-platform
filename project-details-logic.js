...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof window.getStoredData === 'undefined' || typeof projectsData === 'undefined' || typeof donorsData === 'undefined') {
     ...
     /* داخل js/project-details-logic.js - لعرض جدول المصروفات للجمهور */

// دالة رسم جدول المصروفات
const renderExpendituresTable = (expenses) => {
    ...
    if(!expenses || expenses.length === 0) {
        tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;">لم يتم تسجيل مصروفات لهذا المشروع بعد.</td></tr>';
        return;
    }
    ...
    expenses.forEach(exp => {
        ...
        const statusHtml = isVerified 
            ? '<span style="color:#2ecc71; font-weight:bold;"><i class="fas fa-check-circle"></i> مؤكد</span>' 
            : '<span style="color:#f39c12;"><i class="fas fa-hourglass-half"></i> قيد التحقق</span>';

        const confirmHtml = isVerified
            ? `<span style="font-size:0.8em; color:#777;">${exp.verificationDate ? exp.verificationDate.slice(0,10) : ''}</span>`
            : '<span style="color:#ccc;">-</span>';

        const row = document.createElement('tr');
        row.innerHTML = `
            <td data-label="التاريخ">${exp.date}</td>
            <td data-label="البيان">${exp.item}</td>
            <td data-label="المبلغ">${Number(exp.amount).toLocaleString()}</td>
            <td data-label="المسؤول">إدارة المشروع</td>
            <td data-label="المستلم">${exp.recipient}</td>
            <td data-label="الحالة">${statusHtml}</td>
            <td data-label="الفاتورة">${exp.invoiceUrl ? `<a href="${exp.invoiceUrl}" target="_blank" class="invoice-link">عرض</a>` : '-'}</td>
            <td data-label="تأكيد الاستلام">${confirmHtml}</td>
        `;
...
// استدعاء الدالة
renderExpendituresTable(project.expenditures);
